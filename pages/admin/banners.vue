<template>
    <v-container>
        ...
        <v-row>
            <v-col cols="4">
                <v-text-field v-model="searchBanners" :label="$t('search')" solo hide-details append-icon="mdi-table-search" color="secondary">
                </v-text-field>
            </v-col>
            <v-col class="text-right">
                <v-tooltip bottom content-class="bottom">
                    <template v-slot:activator="{ on }">
                        <v-btn text v-on="on" v-show="minimizedBannerDialog" outlined color="primary" @click="maximizeNewBanner()">
        ...
        <v-card flat outlined class="card-table mt-4">
            <v-skeleton-loader class="mx-auto" type=" table: table-heading, table-thead, table-tbody, table-tfoot" :loading="loading">
                <v-data-table :items="banners" :headers="bannersHeaders" :search="searchBanners" sort-by="bannerid" sort-desc class="body-2">
                    ...
                    <template v-slot:[`item.image`]="{item}">
                        <v-img cover max-width="120px" max-height="40px" :src="$config.storage+'/banners%2F' + item.image + '?alt=media'"></v-img>
                    </template>
                    <template v-slot:[`item.imageMobile`]="{item}">
                        <v-img cover max-width="120px" max-height="40px" :src="$config.storage+'banners%2F' + item.imageMobile + '?alt=media'"></v-img>
                    </template>
                    ...
                    <template v-slot:[`item.actions`]="{ item }">
                        <v-tooltip top content-class="top">
                            <template v-slot:activator="{ on }">
                                <v-btn icon color="primary" v-on="on" @click="updateBanner(item)" :disabled="item.status == 3">
                                    <v-icon>edit</v-icon>
                                </v-btn>
                            </template>
                        ...
                        <v-tooltip top content-class="top">
                            <template v-slot:activator="{ on }">
                                <v-btn icon color="error" v-on="on" @click="deleteBanner(item)" :disabled="item.status == 3">
        ...
        <BannersUpdateBannerDialog v-model="updateBannerDialog" :banner="bannerSelected" :user="user" v-if="bannerSelected != null" @finish="listBanners()" @minimize="minimizeBannerDialog()" @cancel="cancelBannerDialog()" />
        <BannersNewBannerDialog v-model="newBannerDialog" :user="user" @finish="listBanners()" @minimize="minimizeBannerDialog()" @cancel="cancelBannerDialog()" />
    </v-container>
    </template>
