        ...
        <v-card flat tile class="card-surface pt-2">
            <v-form v-model="validBrand" v-on:submit.prevent="updateBrand">
                        ...
                        <v-row dense>
                            <v-col>
                                <v-text-field dense outlined class="body-1" background-color="foreground" required
                                    :rules="rulesGlobal.required" v-model="bannerEditable.name" label="Nombre"
                                    color="secondary"></v-text-field>
                                <v-select dense outlined class="body-1" background-color="foreground"
                                    v-model="bannerEditable.category" :items="categories" return-object item-text="name"
                                    label="Categoria" color="secondary"></v-select>
                                <v-text-field dense outlined class="body-1" background-color="foreground"
                                    v-model="bannerEditable.url" label="Link" color="secondary"></v-text-field>
                            </v-col>
                        </v-row>
                        <v-row dense>
                            <v-col>
                                <h5 class="body-1 pa-3">Imagen desktop</h5>
                                <v-file-input dense v-model="bannerEditable.file" placeholder="Seleccione su archivos"
                            ...
                            <v-col>
                                ...
                                <v-file-input dense v-model="bannerEditable.fileMobile" placeholder="Seleccione su archivos"
                                    accept="image/*" color="secondary" background-color="foreground" outlined
                ...
                <v-card-actions class="justify-center background">
                    <v-btn large color="primary" type="submit" :disabled="!validBrand || processingImg"
    ...
    methods: {
        ...
        updateBrand() {
            ...
            this.$fire.firestore.collection('banners').doc(this.bannerEditable.id).update({
                ...
                image: this.bannerEditable.imagePath,
                imageMobile: this.bannerEditable.imageMobilePath
            }).then(result => {
        ...
        async fileInput(file) {
            try {
                if (file && file.name) {
                    ...
                    this.bannerEditable.imagePath = filePath
        ...
        async fileInputMobile(file) {
            try {
                if (file && file.name) {
                    this.processingImg = true;
                    const filePath = `${Date.now()}-${file.name}`;
                    this.bannerEditable.imageMobilePath = filePath
                    await this.$fire.storage.ref()
                        .child('banners')
                        .child(filePath)
                        .put(file);
